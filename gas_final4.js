...
function doPost(e) {
  try {
    ...
    Logger.log('受信データ長: ' + raw.length);
    Logger.log('最初の200文字: ' + raw.substring(0, 200));
    var data = JSON.parse(raw);
    Logger.log('owner: ' + data.owner);
    Logger.log('rows数: ' + data.rows.length);
    Logger.log('写真数(左0): ' + (data.rows[0].leftPhotos ? data.rows[0].leftPhotos.filter(function(p){ return p !== null; }).length : 0));
...
function insertImage(sheet, base64DataUrl, row, col) {
  try {
    ...
    Logger.log('insertImageエラー row=' + row + ' col=' + col + ': ' + err.toString());
